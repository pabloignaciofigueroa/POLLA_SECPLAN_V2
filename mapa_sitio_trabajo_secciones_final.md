...
Historial UI DEFINICION SIMULTANEA (consumidores Stage 1, SOLO LECTURA, sobre la fundacion):
F6 centro de definicion + F7 ranking vivo + F8 cronologia "Que cambio" + F9 clasificacion por
grupo, y 2026-06-23 F10 = barra personal fija en vivo (mobile) en /proximo-partido
(`LivePersonalCard.astro`, alimentada por el recompute de F6, sin canal nuevo; extension a
/tabla y /estadisticas diferida). 2026-06-23 F12 = grafico Carrera de Puntaje DETERMINISTA
(orden estable `dateUtc -> matchNumber -> matchId` en `buildScoreRaceTimeline` + `matchSequence`,
independiente del orden de carga / segundo de finalizacion) + overlay en vivo de los 2 finales
simultaneos (`liveMatches[]` en builder y `score-race.client.js`); SOLO puntaje de partido
(sin bonos 1o/2o), solo lectura, sin segundo `subscribeLiveData`. Detalle por seccion en los
`*.map.md`.

2026-06-23 F11 = ADMIN cierre de grupo + reversion (PRIMER consumidor que ESCRIBE, no solo
lectura): panel `GroupClosePanel` en /admin (detras de `[data-admin-protected]`) que, para
cada grupo en PENDING_CLOSE / FINAL / REOPENED, muestra la tabla final, el 1o/2o y el preview
de bonos (CERO formula nueva: `computeGroupSituation` + `buildGroupBonuses`), con "VALIDAR Y
CERRAR" (confirmacion inline de doble paso) que consolida via RPC `closeGroup` (idempotente,
upsert + version++), "REABRIR" con motivo (campo de texto, no prompt) via `reopenGroup`, y
banner `closureStale` que fuerza la reapertura cuando un resultado corregido ya no coincide con
el cierre congelado. Reusa el UNICO `subscribeLiveData` del KPI de admin (sin canal nuevo);
logica pura testeable en `lib/admin/groupClosePreview.js`. Sin grupos por cerrar el panel queda
...
- DEFINICION SIMULTANEA /tabla (2026-06-24, Estado C/D, SOLO LECTURA): cuando el partido actual tiene su PAR del grupo a la misma hora, /tabla pasa a MODO DUAL. El ranking (izq) NO cambia (F7 ya cuenta todos los vivos). Lo nuevo es presentacional: `SimultaneousWindow.astro` (hero dual = titulo | cards de los 2 partidos por `data-phase` live/pending/official | clasificacion viva del grupo via `computeGroupSituation` con live GATEADO por fase, no cuenta 0-0 preparado) + `SimultaneousPredictions.astro` (matriz por jugador: Partido A/B + clasificados + Pts en vivo = delta de `buildPointLedger` con desglose A/B/CLAS; precision agregada OMITIDA, fallback aprobado). Resolver puro `lib/tabla/resolveDisplayWindow.js` (ancla en el partido actual, agrupa por `dateUtc`/`groupId`, 1..N grupos; tests `tabla-display-window.test.mjs`). `tabla.client.js` `toggleSimultaneousMode` oculta/revela hero/panel normal vs simul por `hidden`; N<=1 byte-igual. Cascarones SSR `hidden` + `<style is:global>` (paleta CLARA). Un solo `subscribeLiveData`. PENDIENTE: QA visual 5 resoluciones. Detalle en `tabla.map.md`.
- Cuando cambiar: formula de puntaje en `scoring-rules.json`/helpers de `lib/tabla`; filas/visual en componentes de `05_tabla`; pipeline en vivo en `tabla.client.js` + `lib/liveMatch/liveMatchState.js`; proyectado/delta/formula en vivo desde `buildPointLedger` (no reimplementar puntaje en la UI); modo simultaneo en `SimultaneousWindow`/`SimultaneousPredictions` + `resolveDisplayWindow`.
...
- DEFINICION SIMULTANEA F8 (2026-06-23): cronologia "Que cambio" SOLO LECTURA (`WhatChangedFeed.astro`) dentro del Centro de definicion, DEBAJO de la tabla viva. Narra las DIFERENCIAS entre snapshots (gol -> reordenamiento -> impacto por jugador -> "sin cambios") mas nuevo arriba, con filtros Todos/Mi jugador y cola "N cambios nuevos". El motor de diff es `lib/statistics/buildChangeEvents.js` (puro + `deriveRanking`): compara el snapshot anterior vs el actual (effectiveByMatch/situations/byPlayer/ranking que el recompute YA produjo) y NO recalcula puntaje (cero formula nueva). La cronologia se arma por DIFF EN CLIENTE, NO por el `ts` del libro (best-effort). Eventos de 1o/2o SOLO para grupos EN DEFINICION (gate heredado: `situations` solo trae grupos con `definitionStarted`/final). Reusa el MISMO `subscribeLiveData(recompute)` de F6 (un solo dueno; no abre segundo canal). Sin vivo / sin centro activo: feed oculto = /proximo-partido identico a hoy. NO usa la tabla persistida `polla_match_event` (fuera de alcance). Tests: `tests/change-events.test.mjs`. Detalle en `proximo-partido.map.md`.
- Gotcha local: algunas banderas/escudos se inyectan con `innerHTML`; el estilo debe ir inline o en reglas globales acotadas. F8: los items del feed se crean en runtime por `innerHTML`, su CSS va en `<style is:global>` anclado a `[data-what-changed-feed]`.
- DEFINICION SIMULTANEA F10 (2026-06-23, barra personal fija mobile, SOLO LECTURA): `LivePersonalCard.astro` = barra FIJA al fondo del viewport, SOLO en mobile (<=720px) y SOLO cuando hay un grupo EN DEFINICION (mismo gate que el Centro de F6). Muestra "TU PUNTAJE EN VIVO" + proyectado protagonista + "N oficiales + M en juego"; toque -> desglose corto (Partidos +X / Clasificacion +Y de `me.match`/`me.group`, matriz Final 1/Final 2/1o/2o de `me.lines`, link a la cronologia F8). Es la version barra-fija-mobile de la `YourImpactCard` de F6. Se alimenta del MISMO `subscribeLiveData`/`recomputeCenter` (un solo dueno; NO abre segundo canal): `updateLivePersonalCard(...)` lee `me = buildPointLedger.byPlayer[selectedPlayerId]` -> CERO formula nueva en la UI; el bono de grupo ya viene gateado (bloqueados aportan 0). Markup SSR estatico (`[data-live-personal-card]`, `data-active="false"` por defecto -> oculto = cero regresion en desktop / sin vivo). Estados: sin identidad -> CTA "Elige tu jugador" -> /jugador; delta 0 con vivo -> "+0 en juego" gris. No tapa contenido: `.proximoPartidoSection[data-lpc-active="true"] .contentShell` reserva padding-bottom + `env(safe-area-inset-bottom)`. a11y: `aria-expanded`, `aria-hidden`, `aria-live="polite"`, foco visible; animacion <300ms con `prefers-reduced-motion`. Identidad `polla:selectedPlayerId` (solo lectura). Extension a /tabla y /estadisticas DIFERIDA (no era reuso limpio del owner sin regresion; ver `comandas_F10_tarjeta_personal/22_pasoC_pulido_extension.md`). Detalle en `proximo-partido.map.md`.
- Cuando cambiar: partido destacado y lectura editorial en data/mock y logica de seccion. Barra personal (F10): `LivePersonalCard.astro` + `updateLivePersonalCard`/`fillLpcDetail` en `proximo-partido.client.js` (alimentar solo desde el recompute existente, nunca un canal nuevo).
...
- Carrera de Puntaje determinista + overlay multi-final (GRÁFICO, F12, 2026-06-23): el historico cerrado del grafico se reconstruye con orden estable TOTAL `dateUtc -> matchNumber -> matchId` (comparador `byStableMatchOrder` en `buildScoreRaceTimeline`), NUNCA por el segundo de finalizacion del Admin ni por el orden del array de `officialResults`. El MISMO criterio (incluido el desempate final `matchId`) vive en `matchSequence.buildMatchSequence` (numero correlativo del eje X), para que nodo y etiqueta P-N no se desincronicen. Mismo set de resultados -> grafico identico (orden, clusters, totales). El overlay EN VIVO acepta `liveMatches[]` (los 2 finales simultaneos de un grupo a la vez), no solo el `liveMatchState` singular legado: `score-race.client.js` resuelve TODOS los `liveSnapshot.liveMatches` (gate por `resolveLiveMatchPhase`, mapeo `*TeamScore`→`*Score`, dedup por `matchId`) y los pasa al builder; el oficial siempre gana sobre el vivo del mismo match; con N=1 el vivo sigue siendo el ultimo nodo (cero regresion). SOLO LECTURA y SIN segundo `subscribeLiveData` (dueno unico `estadisticas.client.js`). ALCANCE: el grafico suma SOLO puntaje de partido 5/3/1/0 (fuente unica `liveScoring.js`); NO incorpora bonos 1o/2o (cambiaria la semantica; decision aparte). Tests nuevos en `tests/score-race-timeline.test.mjs` (barajar input -> salida identica, simultaneas, desempates, vivo multiple, vivo->oficial, compat N=1).
...
- 2026-06-23 (comanda F11 "Admin cierre de grupo + reversion", arbol `comandas_F11_admin_cierre`): /admin estrena el panel de CIERRE de grupo `GroupClosePanel` (PRIMER consumidor que ESCRIBE; el resto de F6-F12 es solo lectura). (a) UI: `GroupClosePanel.astro` (shell SSR oculto dentro de `[data-admin-protected]`; CSS `<style is:global>` anclado a `[data-group-close]` porque el cuerpo nace por innerHTML en runtime, gotcha de scope) montado en `AdminSection.astro` despues de `MatchProgressPanel`. El client (`admin.client.js` -> `initGroupClosePanel`) pinta una tarjeta por grupo en PENDING_CLOSE / FINAL / REOPENED con cabecera + badge de estado, tabla final (de `computeGroupSituation.standings`, con insignia 1o/2o), preview de bonos (de `buildGroupBonuses.byGroup`: aciertos 1o +1 / 2o +3 y total a consolidar, rotulado "preview (aun provisional)") y acciones. (b) "VALIDAR Y CERRAR" -> `confirmDialog` (doble paso inline, nunca alert/confirm/prompt) -> RPC `closeGroup(groupId, first, second, standings)` (upsert por group_id, version++, idempotente: dos clics no duplican; el motor reconstruye desde lineas). En FINAL coherente el boton NO se ofrece (idempotencia en UI); en FINAL stale se ofrece "Recerrar (corregido)". (c) "REABRIR" (destructivo, solo en FINAL) -> campo de texto de MOTIVO en el markup (NO prompt nativo) + `confirmDialog` -> RPC `reopenGroup(groupId, reason)` (state reopened, version++; los bonos vuelven a provisional sin doble conteo). (d) `closureStale`: en cada snapshot, para FINAL, se lee `situation.closureStale` (1o/2o congelado != recomputado, o un partido del grupo dejo de ser oficial); si true, banner rojo "Grupo cerrado desactualizado: reabrir y recalcular" + se resalta REABRIR. (e) CERO formula nueva: todo (estado, 1o/2o, bonos, stale) sale de la fundacion (`groupState.js`/`groupBonuses.js`); la logica pura de deteccion/preview/idempotencia-UI vive en `lib/admin/groupClosePreview.js` (testeable offline). (f) Un solo dueno del dataset: el panel NO abre un `subscribeLiveData` propio; cuelga del UNICO subscribe del KPI de admin (`initOfficialResultsKpi` ahora reenvia el snapshot al panel). Escritura SOLO por RPC security definer con sesion admin (P0001 con token dummy, no PGRST202); sin policies publicas de INSERT/UPDATE; `MULTI_LIVE_WRITE_ENABLED` NO se toca (es Stage 2, F11 no lo necesita). Sin grupos por cerrar el panel queda oculto -> /admin identico a hoy (cero regresion). 11 tests offline nuevos (`tests/group-close-preview.test.mjs`: deteccion de estados, preview de bonos, cierre idempotente version++ sin duplicar, canOfferClose/Reopen, closureStale fuerza recerrar/reabrir, reapertura sin doble conteo). Suite 125/125, build 11 paginas. El SQL (`polla_group_closure` + RPC `polla_close_group`/`polla_reopen_group`) YA existe en `supabase/migrations/20260622120100_group_closure.sql` y `supabase/remote/apply_group_closure.sql` (NO editado: estaba correcto y en paridad); NO se ejecuto contra remoto. PASO MANUAL PENDIENTE del operador humano: aplicar `apply_group_closure.sql` en Supabase remoto en ventana SIN partido vivo, con backup (sin service key en el repo; verificar P0001 por REST con anon key). Pendiente: F13 simulacion, Stage 2 (multi-write de marcadores para los 2 finales simultaneos).
- 2026-06-23 (comanda F8 "Cronologia Que cambio", arbol `comandas_F8_que_cambio`): /proximo-partido estrena la cronologia "Que cambio" (SOLO LECTURA) DENTRO del Centro de definicion (F6), DEBAJO de la tabla viva. Narra las DIFERENCIAS entre snapshots consecutivos (la pelicula de la jornada: gol -> reordenamiento de grupo -> impacto por jugador -> "sin cambios"), mas nuevo arriba, con filtros Todos/Mi jugador y cola "N cambios nuevos". (a) Motor de diff PURO nuevo: `lib/statistics/buildChangeEvents.js` (+`deriveRanking`): dado el snapshot ANTERIOR y el ACTUAL (`{effectiveByMatch, situations, byPlayer, ranking}` que el recompute YA produjo con F0-F5) devuelve los eventos nuevos. GOAL = marcador efectivo de un partido cambio; REORDER = cambio de 1o/2o/orden en un grupo EN DEFINICION; IMPACT = cambio de `projected` (descompuesto en `+N por marcador` / `+N por 1o/2o` leyendo `ledger.lines`) y/o de puesto de ranking, con signo (verde sube/rojo baja/gris 0); NONE = el "0 se explica" (jugador estable, opt-in via `forPlayerId` para el filtro Mi jugador). Orden deterministico dentro de un snapshot: goles -> reordenamientos -> impactos -> none; entre snapshots, orden de LLEGADA (NO el `ts` del libro, que es best-effort). CERO formula nueva: toda cifra sale del ledger/situations. (b) Gate heredado: los eventos de 1o/2o SOLO para grupos EN DEFINICION (`situations` solo trae grupos con `definitionStarted`/final); grupos BLOQUEADOS (fechas 1-2) nunca generan reorder ni linea de grupo (a lo sumo un gol del partido). (c) UI: `WhatChangedFeed.astro` (shell SSR oculto, lista `aria-live="polite"`, filtros con `aria-pressed`, pildora de cola flotante) montado dentro de `GroupDefinitionCenter`; el client (`proximo-partido.client.js`) reusa el UNICO `subscribeLiveData(recompute)` de F6 (NO abre segundo canal): mantiene `prevChangeSnapshot`, acumula eventos (cap 200), agrupa los impactos de un mismo lote cuando son >=4 en un resumen "N jugadores afectados" expandible (anti-saturacion), animacion de entrada <300ms transform/opacity con `prefers-reduced-motion` respetado, limpieza de timer. CSS de los items (creados por innerHTML en runtime) en `<style is:global>` anclado a `[data-what-changed-feed]` (gotcha de scope). (d) Cero regresion: sin grupo en definicion el centro (y el feed anidado) queda oculto -> /proximo-partido identico a hoy; el diff avanza igual sin mostrar nada. NO se implementa la tabla persistida `polla_match_event` (fuera de alcance; la cronologia es diff en cliente). 11 tests nuevos (`tests/change-events.test.mjs`: gol entre snapshots, reorder solo en definicion, grupo bloqueado sin 1o/2o, impacto descompuesto con signo, "sin cambios", orden deterministico, deriveRanking). Suite 105/105, build 11 paginas. Pendiente: F10 movil, F11 admin de cierre, Stage 0/2.
